...
fn main() {
  if is_linux() {
    ...
    println!("Config directory: {}", config_dir.ok_or(libc::ENOENT).unwrap());
      ...
      .send()
    {
      Ok(response) => {
        ...
        if response.status().is_success() {
          ...
          match response.text() {
              Ok(text) => eprintln!("Error response body: {}", text),
              Err(_) => eprintln!("Could not read error response body as text."),
          }
        }
      }
      Err(e) => {
...
fn get_release_url(username: &str, repo: &str, tag: Option<&str>) -> String {
  match tag {
    Some(tag) => format!(
      "https://api.github.com/repos/{}/{}/releases/tags/{}", username, repo, tag),
    None => format!("https://api.github.com/repos/{}/{}/releases/latest", username, repo)
  }
}
